...
class challenge01 {
    ...
    public static String challenge(String txt) {
        LinkedHashMap<String, Integer> animals = new LinkedHashMap<>();
        String[] words = txt.toLowerCase().split(" ");
        for (String word : words) {
            animals.put(word, animals.getOrDefault(word, 0) + 1);
        }
        StringBuilder result = new StringBuilder();
        animals.forEach((word, count) -> {
            result.append(word).append(count);
        });
